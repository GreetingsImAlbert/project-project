-- Task 9: atomic insertion of an already validated, remapped project import.
-- This later definition includes all five project visibility flags and keeps
-- imported projects private regardless of the exported values.
...
begin
  ...
  if jsonb_typeof(v_project) is distinct from 'object'
    or (select count(*) from jsonb_object_keys(v_project)) <> 13
    or not (
      v_project ? 'id'
      and v_project ? 'avatar'
      and v_project ? 'name'
      ...
      and v_project ? 'public_tasks_enabled'
      and v_project ? 'public_journal_enabled'
      and v_project ? 'public_money_enabled'
  ...
  if jsonb_typeof(v_project->'is_public') is distinct from 'boolean'
    ...
    or jsonb_typeof(v_project->'public_tasks_enabled') is distinct from 'boolean'
    or jsonb_typeof(v_project->'public_journal_enabled') is distinct from 'boolean'
    or jsonb_typeof(v_project->'public_money_enabled') is distinct from 'boolean'
    or v_project->>'is_public' is distinct from 'false'
    or v_project->>'public_files_enabled' is distinct from 'false'
    or v_project->>'public_tasks_enabled' is distinct from 'false'
    or v_project->>'public_journal_enabled' is distinct from 'false'
    or v_project->>'public_money_enabled' is distinct from 'false' then
  ...
  if jsonb_typeof(v_project->'avatar') not in ('string', 'null') then
    raise exception 'invalid project avatar';
  end if;
  if jsonb_typeof(v_project->'avatar') = 'string'
    and v_project->>'avatar' not in ('dot', 'ring', 'square', 'diamond', 'triangle', 'cross', 'bars', 'grid')
    and v_project->>'avatar' !~* ('^projects/' || v_project_id::text || '/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.img$') then
    raise exception 'invalid project avatar';
  end if;
  ...
  insert into public.projects (
    id, avatar, name, description, owner_id, currency, is_public, public_files_enabled,
    public_tasks_enabled, public_journal_enabled, public_money_enabled,
