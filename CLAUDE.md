...
**No-reload mutations:** BOM add/edit/delete (`BomTable.svelte`), file move/copy (`FileList.svelte`), and folder create/delete (`FileBrowser.svelte`) all call their API routes via `fetch` and patch local `$state` on success instead of relying on a server redirect + full page reload. Because of this, the corresponding API routes (`api/bom/**`, `api/files/[fileId]/{move,copy}`, `api/projects/[id]/folders/**`) return `Response.json(row)` (or `204`) on success instead of `redirect(...)` — don't reintroduce `redirect()` in these routes, since no caller follows it as a page navigation anymore. File upload (`UploadForm.svelte`) is the one exception still doing `location.reload()` on success.
