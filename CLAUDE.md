...
**Project pages** (`src/pages/projects/[id].astro`, `[id]/files.astro`, `[id]/money.astro`): three separate routes, not one combined page — Overview (`[id].astro`, currently empty — a placeholder for future project-level content), Files, and Money (hosts the BOM section — only the nav label/route is `Money`; `BomTable.svelte`'s own heading is still `Bill of Materials`). Each page repeats the same `Astro.locals.user` redirect-to-login check and project/members/role fetch (per the no-route-group-guard convention above), then fetches only the data it needs (Files also loads folders/files, Money also loads `bom_items`) and renders `ProjectShell.astro` — a shared wrapper (`.project-shell` two-column flex layout + the '← Back to projects' link) that takes `activePage: 'overview' | 'files' | 'money'` and renders `ProjectSidebar.astro` plus a `<slot />` for the page's own content in `.project-main`. `ProjectSidebar.astro` (project title, description, an inline members list — no separate `MembersPanel` component anymore, since the sidebar's member format (`display name (role)`, no email, no header borders) diverged enough from a general-purpose panel that it wasn't worth sharing) takes the same `activePage` prop and renders real `<a href="/projects/[id]/...">` nav links (not same-page anchors like the old `#files`/`#bom` — there's nothing to anchor-scroll to anymore since each section is its own page), underlining whichever one matches `activePage` the same way `Header.astro`'s active nav link works. The sidebar has no fold/collapse (tried a `<details>`/`<summary>` toggle, then a plain-button + `.collapsed`-class toggle, both removed — decided it wasn't worth the interaction cost) and stays `position: sticky` so it stays visually in place while the main column scrolls — there's no independent scroll container per column, so keep the sidebar's content short enough that it doesn't grow taller than the viewport, or the sticky effect stops looking like "only the main column scrolled." On mobile (`.project-shell`'s `flex-direction: column` at ≤768px), remember to also set `align-items: stretch` there — `flex-start` (needed on desktop so the sidebar doesn't stretch to the main column's height) flips to controlling the horizontal axis once the direction is column, and without `stretch` the main column sizes to its widest content's natural width (e.g. the Money page's BOM table colgroup) instead of the viewport. `FileBrowser.svelte`'s `hrefFor()`/`pushState` folder-navigation URLs now point at `/projects/[id]/files?folder=...` instead of the old bare `/projects/[id]?folder=...`.
