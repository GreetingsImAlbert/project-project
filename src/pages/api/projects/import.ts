...
import { CUSTOM_AVATAR_BUCKET } from '../../../lib/avatars';
...
class ProjectImportStorageError extends Error {}

interface StagedProjectPicture {
	path: string;
}
...
async function deleteStagedProjectPicture(
	admin: ReturnType<typeof getSupabaseAdmin>,
	picture: StagedProjectPicture | null,
): Promise<void> {
	if (!picture) return;
	const { error } = await admin.storage.from(CUSTOM_AVATAR_BUCKET).remove([picture.path]);
	if (error) console.error(`[project-import] Could not clean up staged project picture ${picture.path}: ${error.message}`);
}
...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	let stagedPicture: StagedProjectPicture | null = null;
	...
	try {
		...
		if (remapped.projectPicturePath) {
			...
			if (!picture || !validated.projectPictureBytes) {
				throw new ProjectImportError('Validated project picture content is missing.');
			}
			stagedPicture = { path: remapped.projectPicturePath };
			const { error: pictureError } = await admin.storage.from(CUSTOM_AVATAR_BUCKET).upload(
				stagedPicture.path,
				validated.projectPictureBytes,
				{
					contentType: picture.mime_type,
					cacheControl: '31536000',
					upsert: false,
				},
			);
			if (pictureError) {
				console.error('[project-import] Project picture upload failed', pictureError);
				throw new ProjectImportStorageError('Could not store the imported project picture.');
			}
		}
		if (remapped.payload.files.length > 0) {
			if (!env.R2_BUCKET) {
				const failedPicture = stagedPicture;
				stagedPicture = null;
				await deleteStagedProjectPicture(admin, failedPicture);
				return json({ message: 'File storage is unavailable.' }, 503);
			}
		...
		if (duplicateProject) {
			...
			const duplicatePicture = stagedPicture;
			staged = null;
			stagedPicture = null;
			if (duplicateStage && env.R2_BUCKET) await deleteStagedProjectFiles(env.R2_BUCKET, duplicateStage);
			await deleteStagedProjectPicture(admin, duplicatePicture);
			return json({
				projectId,
				name: validated.projectName,
				fileBytes: limits.fileBytes,
		...
		if (adminClient) await deleteStagedProjectPicture(adminClient, stagedPicture);
		if (error instanceof ProjectImportError) return json({ message: error.message }, 400);
		if (error instanceof ProjectImportStorageError) return json({ message: error.message }, 502);
