...
function withSecurityHeaders(response: Response, pathname: string, hostname: string): Response {
    ...
    if (hostname === 'p2testing.albertmendoza.com') {
...
export const onRequest = defineMiddleware(async (context, next) => {
    if (!isAppPath(context.url.pathname)) {
        return withSecurityHeaders(await next(), context.url.pathname, context.url.hostname);
    }
    ...
    try {
        ...
        if (context.locals.user && pendingDeletionAt && !isPendingDeletionAllowed(context.url.pathname)) {
            return withSecurityHeaders(context.redirect('/account/pending-deletion'), context.url.pathname, context.url.hostname);
        }
        ...
        return withSecurityHeaders(unexpectedErrorResponse(context.url.pathname, reportId), context.url.pathname, context.url.hostname);
    }
});
