...
import { CUSTOM_AVATAR_MAX_BYTES, isAvatarId, parseCustomAvatarBytes } from './avatars';
...
const PROJECT_PICTURE_MIME_TYPES = new Set<ProjectPictureMimeType>(['image/jpeg', 'image/png', 'image/webp']);
...
export interface ValidatedProjectImport {
	...
	projectName: string;
	fileBytes: number;
	projectPictureBytes: Uint8Array<ArrayBuffer> | null;
}
...
function projectPictureDescriptor(value: unknown): ProjectPictureDescriptor | null {
	if (value === null) return null;
	const row = object(value, 'projectPicture');
	if (row.kind === 'builtin') {
		exactKeys(row, 'projectPicture', ['kind', 'id']);
		if (!isAvatarId(row.id)) fail('Invalid manifest: projectPicture built-in id is unsupported.');
		return { kind: 'builtin', id: row.id };
	}
	if (row.kind === 'custom') {
		exactKeys(row, 'projectPicture', ['kind', 'archive_path', 'mime_type', 'content_size_bytes', 'sha256']);
		if (row.archive_path !== PROJECT_PICTURE_ARCHIVE_PATH) fail('Invalid manifest: projectPicture archive path is unsupported.');
		if (typeof row.mime_type !== 'string' || !PROJECT_PICTURE_MIME_TYPES.has(row.mime_type as ProjectPictureMimeType)) {
			fail('Invalid manifest: projectPicture MIME type is unsupported.');
		}
		const contentSize = number(row.content_size_bytes, 'projectPicture.content_size_bytes', true);
		if (contentSize < 1 || contentSize > CUSTOM_AVATAR_MAX_BYTES) fail('Invalid manifest: projectPicture size is outside the image limit.');
		const sha256 = string(row.sha256, 'projectPicture.sha256', 64);
		if (!SHA256_PATTERN.test(sha256)) fail('Invalid manifest: projectPicture checksum is invalid.');
		return {
			kind: 'custom',
			archive_path: PROJECT_PICTURE_ARCHIVE_PATH,
			mime_type: row.mime_type as ProjectPictureMimeType,
			content_size_bytes: contentSize,
			sha256,
		};
	}
	fail('Invalid manifest: projectPicture kind is unsupported.');
}
...
function validateManifest(raw: unknown): ProjectExportManifest {
	...
	return { ...baseManifest, version: PROJECT_EXPORT_VERSION_V1 };
}
...
async function validateProjectPictureEntry(
	archive: ParsedProjectZip,
	manifest: ProjectExportManifest,
	...
	if (!pictureEntry || pictureEntry.isDirectory) fail('Invalid P2 archive: custom project picture content is missing.');
	if (pictureEntry.bytes.length !== descriptor.content_size_bytes) fail('Invalid P2 archive: project picture size does not match the manifest.');
	const parsed = parseCustomAvatarBytes(pictureEntry.bytes);
	if (!parsed || parsed.mimeType !== descriptor.mime_type) fail('Invalid P2 archive: project picture signature or MIME type is invalid.');
	const checksum = await sha256Hex(pictureEntry.bytes);
	if (checksum.toLowerCase() !== descriptor.sha256.toLowerCase()) fail('Invalid P2 archive: project picture checksum failed.');
	return pictureEntry.bytes;
}
...
export async function validateP2ProjectArchive(bytes: Uint8Array<ArrayBuffer>): Promise<ValidatedProjectImport> {
	...
	const projectPictureBytes = await validateProjectPictureEntry(archive, manifest);
	...
	for (const entry of archive.entries.values()) {
		...
		if (entry.name === PROJECT_PICTURE_ARCHIVE_PATH) continue;
	...
	return { manifest, archive, projectName: manifest.project.name, fileBytes: archiveFileBytes, projectPictureBytes };
}
